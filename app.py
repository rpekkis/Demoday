...
if start_opt or start_sub:
    ...
    for i in range(len(flight_data)):
        ...
        fig_body.add_shape(type="path",
            ...
            fillcolor="rgba(255, 255, 255, 0.05)", line=dict(color="rgba(255, 255, 255, 0.2)", width=2)
        )
        
        locs = {"Neck": [50, 88], "Back": [50, 70], "Core": [50, 55], "Glutes": [50, 42], "Quads": [50, 25]}
        for m, loc in locs.items():
            fig_body.add_trace(go.Scatter(
                x=[loc[0]], y=[loc[1]], mode='markers',
                marker=dict(size=35 + (strains[m]/5), color=get_h_color(strains[m]), line=dict(width=2, color='white')),
