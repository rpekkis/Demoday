...
if start_opt or start_sub:
    ...
    img_path = "body.png" # MUISTA ETTÄ TÄMÄ ON KANSIOSSA
    ...
    for i in range(len(flight_data)):
        ...
        # Sijoitetaan lihakset tarkasti kuvan päälle (koordinaatit [x, y] 0-100 koordinaatistossa)
        locs = {
        ...
        if os.path.exists(img_path):
            encoded_image = get_encoded_image(img_path)
            prefix = "data:image/png;base64,"
            
            fig_body.add_layout_image(
                dict(
                    source=prefix + encoded_image,
                    x=50, y=50, sizex=100, sizey=100,
                    xanchor="center", yanchor="middle", sizing="contain", layer="below"
                )
            )
        else:
            fig_body.add_annotation(x=50, y=50, text="HUMAN DYNAMICS", font=dict(size=40, color="gray"), showarrow=False)
