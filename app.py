...
def generate_aggressive_profile():
    ...
    return np.clip(g + np.random.normal(0, 0.02, 300), 1.0, 9.5)
    ...
    st.markdown(f"""
        <div style="margin-bottom: 8px;">
            <div style="display: flex; justify-content: space-between; margin-bottom: 2px;">
                <span style="font-size: 12px; font-weight: bold; color: #ddd;">{label.upper()}</span>
                <span style="font-size: 12px; color: #eee;">{value}% MVC</span>
            </div>
            <div style="background-color: #333; border-radius: 4px; width: 100%; height: 8px;">
                <div style="background-color: {color}; width: {value}%; height: 8px; border-radius: 4px; transition: width 0.1s;"></div>
            </div>
        </div>
    """, unsafe_allow_html=True)

# --- SENSORIPALLOJEN SIJOITTELU ---
def get_sensor_html(strains):
    # Säädetyt koordinaatit [% ylhäältä, % vasemmalta]
    # Niska on korkealla, Core keskellä, Quads (reidet) alhaalla
    locs = {
        "Neck": [18, 50], 
        "Back": [42, 50], 
        "Core": [53, 50],
    ...
    for m, pos in locs.items():
        ...
        # Pallo hehkuu ja vaihtaa väriä ilman välkkymistä
...
with col_heat:
    ...
    st.markdown("---")
    # TÄRKEÄÄ: Käytetään nimeä 'bars' koko koodissa
    bars = {m: st.empty() for m in ["Neck", "Back", "Core", "Glutes", "Quads"]}

with col_charts:
    st.subheader("Flight Telemetry (Unit: G)")
...
if start_opt or start_sub:
    ...
    if os.path.exists(img_path):
        ...
        st.error("body.png missing! Varmista että tiedosto on samassa kansiossa.")
        st.stop()
    ...
    for i in range(len(flight_data)):
        ...
        # --- VAKAA HEATMAP-PÄIVITYS ---
        heatmap_placeholder.markdown(f"""
            <div style="display: flex; justify-content: center; background-color: #0e1117; padding: 15px; border-radius: 10px; border: 1px solid #333;">
                <div style="position: relative; width: 100%; max-width: 280px;">
                    <img src="{img_src}" style="width: 100%; display: block; opacity: 0.4;">
                    {get_sensor_html(strains)}
                </div>
            </div>
        """, unsafe_allow_html=True)

        # --- PÄIVITETÄÄN PALKIT (Käytetään oikeaa muuttujaa 'bars') ---
        ...
        g_metric.metric("LIVE LOAD", f"{current_g:.1f} G", delta=f"{mode}")
